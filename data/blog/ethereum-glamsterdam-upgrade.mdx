...
The first is EIP-7732, often called enshrined proposer-builder separation. In simple terms, it splits the work of agreeing on a block from the heavier work of fully checking the transactions inside it. Ethereum's roadmap explains that this widens the window for spreading block data across the network from roughly two seconds to about nine seconds, which gives the network more breathing room to handle larger amounts of data safely. Both EIP-7732 and the second proposal are currently listed as Draft status.

The second is EIP-7928, or block-level access lists. This proposal asks each block to declare in advance which accounts and data it will touch. According to the EIP, that advance map lets nodes read data and process many transactions in parallel rather than strictly one after another, since most transactions do not overlap.
